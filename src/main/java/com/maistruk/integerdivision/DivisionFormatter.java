...
public class DivisionFormatter {
    ...
    private int formatHeader(DivisionResult divisionResult, StringBuilder buildHeader) {
        ...
        buildHeader.append(repeatChar(SPACE,
                countDigits(dividend) - countDigits(divisionStep.getSubtrahend())));
        buildHeader.append("|" + repeatChar(DASH, countDigits(quotient))).append(lineSeparator());
        buildHeader.append(repeatChar(SPACE, amountOfSpaces)
                + repeatChar(DASH, countDigits(divisionStep.getSubtrahend())));
        buildHeader.append(repeatChar(SPACE,
                countDigits(dividend) - countDigits(divisionStep.getSubtrahend())));
    ...
    private int formatSteps(DivisionResult divisionResult, StringBuilder buildBody, int amountOfSpaces) {
        for (int i = 1; i < divisionResult.divisionSteps.size(); i++) {
            ...
            buildBody.append(repeatChar(SPACE, amountOfSpaces) + "_" + divisionStep.getMinuend()).append(lineSeparator());
            buildBody.append(repeatChar(SPACE, ++amountOfSpaces) + divisionStep.getSubtrahend()).append(lineSeparator());
            buildBody
                    .append(repeatChar(SPACE, amountOfSpaces)
                            + repeatChar(DASH, countDigits(divisionStep.getSubtrahend())))
